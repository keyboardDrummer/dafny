...
public class DiagnosticMigrationTest : ClientBasedLanguageServerTest {
  ...
  public async Task ResolutionDiagnosticsContainPreviousVerificationResultsWhenCodeIsInsertedAfter() {
    ...
    Assert.Equal(IdeStateManager.OutdatedPrefix + verificationDiagnostics[0].Message, resolutionDiagnostics[0].Message);
  }
  ...
  { 
    ...
    Assert.Equal(IdeStateManager.OutdatedPrefix + verificationDiagnostics[0].Message, resolutionDiagnostics1[0].Message);
